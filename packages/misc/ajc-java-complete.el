...
(defcustom ajc-tag-file "~/.elisp/misc/java_base.tag"
  "the tag file is  used for java complete ,it  is generate by a Tags.java ,
...
before that it will use y-or-n-p ask user to confirm "
  (let ((import-class-buffer "*ajc-import-java-class*")
        ...
        (java-buffer (current-buffer))(java-window) (count 1)
	)
    (setq case-fold-search nil)
    (if (and import-class-items-list (> (length import-class-items-list) 1))
        (progn
	      ...
	      (progn
		(insert (concat  (int-to-string count)  ".  " (car (ajc-split-pkg-item-by-pkg-ln  (nth 1 ele ))) "." (car ele)  "\n"))
		(incf count)
		)
	      )
	  ...
	  (add-to-list 'user-confirmed-class-items-list (nth (- (read-char "Your choice: ") 49) import-class-items-list))
	...
	(progn
	  (with-current-buffer java-buffer
	    (ajc-insert-import-at-head-of-source-file-without-confirm import-class-items-list))
	  )
      )
    ))
...
(defun organize-imports ()
  ...
  (let ((begin) (end))
    (save-excursion
      ...
      (setq begin (point))
      (end-of-buffer)
      (re-search-backward "^[ \t]*import[ \t]+[a-zA-Z0-9_\\.\\*]+[ \t]*;")
      (end-of-line)
      (setq end  (point))
      ...
      (beginning-of-line)
      (setq end  (point))
      (uniquify-all-lines-region begin end)
      )
    )
  )
