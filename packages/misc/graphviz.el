...
;;; dot-mode.el --- Mode for the dot-language used by graphviz (att).
...
;; Homepage: http://users.skynet.be/ppareit/projects/dot-mode/dot-mode.html
;; Created: 28 Oct 2002
...
;;
;; To use dot-mode, add 
;; (load-file "PATH_TO_FILE/dot-mode.el") 
;; to your ~/.emacs(.el) or ~/.xemacs/init.el
;;
;; The dot-mode will do font locking, indentation, preview of graphs
...
;; 28/10/2002: derived dot-mode from fundamental-mode, started 
;;             implementing font-lock.

;;; Code:

(defconst dot-mode-version "0.3.6"
  "Version of `dot-mode.el'.")
...
(defvar dot-mode-abbrev-table nil
  "Abbrev table in use in Graphviz Dot mode buffers.")
(define-abbrev-table 'dot-mode-abbrev-table ())
...
\\<dot-mode-map>\\[graphviz-dot-complete-word] will toggle the possible
...
(if dot-mode-map
  ...
  (let ((map (make-sparse-keymap)))
    ...
    (setq dot-mode-map map)
    ))

;;; Syntax table
(defvar dot-mode-syntax-table nil
  "Syntax table for `dot-mode'.")

(if dot-mode-syntax-table
    ()
...
(defvar graphviz-dot-font-lock-keywords
  ...
  "Keyword highlighting specification for `dot-mode'.")

;;;###autoload
(defun dot-mode ()
  "Major mode for the dot language. \\<dot-mode-map> 
...
`dot-mode-hook' with no args, if that value is non-nil."
  (interactive)
  (kill-all-local-variables)
  (use-local-map dot-mode-map)
  (setq major-mode 'dot-mode)
  (setq mode-name "dot")
  (setq local-abbrev-table dot-mode-abbrev-table)
  (set-syntax-table dot-mode-syntax-table)
  (set (make-local-variable 'indent-line-function) 'graphviz-dot-indent-line)
  (set (make-local-variable 'comment-start) "//")
  (set (make-local-variable 'comment-start-skip) "/\\*+ *\\|//+ *")
  (modify-syntax-entry ?# "< b" dot-mode-syntax-table)
  (modify-syntax-entry ?\n "> b" dot-mode-syntax-table)
...
(and (condition-case nil
       ...
       (error nil))
     (easy-menu-define
      dot-menu dot-mode-map "Graphviz Mode menu"
      '("Graphviz"
...
(add-to-list 'auto-mode-alist '("\\.gv\\'" . dot-mode))
