...
(defun wg-mode-line-string ()
  ...
  (let ((wg (wg-current-workgroup t))
        ...
        (total-length (length (wg-workgroup-list-or-error))))
    (cond (wg (wg-fontify " "
                (:brace wg-mode-line-decor-left-brace)
                (:mode (concat (wg-workgroup-name wg) "/" (number-to-string total-length)))
                (if wg-flag-modified
                    (concat
...
(defun wg-new-default-workgroup-name ()
  ...
  (let ((names (wg-workgroup-names t)) (index -1) result)
    (while (not result)
      ...
      (let ((new-name (sw/pick-random-name)))
        (unless (member new-name names)
          (setq result new-name))))
    result))
...
that, use `wg-clone-workgroup'."
  (interactive)
  (wg-switch-to-workgroup (wg-make-and-add-workgroup (wg-new-default-workgroup-name) t)))
