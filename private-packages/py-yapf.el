...
(defun py-yapf-enable-on-save ()
  ...
  (interactive)
  (add-hook 'before-save-hook 'py-yapf-on-save nil t))

(defun py-yapf-on-save()
  (interactive)
  (setq py-yapf-on-save-count (1+ py-yapf-on-save-count))
  (when (> py-yapf-on-save-count 2)
    (py-yapf-buffer)
    (setq py-yapf-on-save-count 0)))

(setq py-yapf-on-save-count 0)
(make-variable-buffer-local 'py-yapf-on-save-count)
;; BEGIN GENERATED -----------------
