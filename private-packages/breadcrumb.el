...
(defconst bc--menu-table-offset 2)
...
(defcustom bc-bookmark-limit 64
  "*Maximum numbers of breadcrumb bookmarks to keep in the queue."
  :type 'integer
  :group 'breadcrumb)

(defun bc-get-bookmark-file ()
  (concat switch-desktop-session-dir (switch-desktop-get-current-name) "/.breadcumb"))
  ...
  (let ((type (bc-get-buffer-type)))
    (if (eq type bc--type-unsupported)
      ...
      (let ((filename (bc-get-buffer-filename type))
            ...
            (anno (concat (if (fboundp 'which-function)
                                    (which-function)
                                  "nil"
        ...
        (if (or (null filename) (null position))
          ...
          (bc-bookmarks-add (bc-bookmark-new anno type filename position))
          (setq *bc-current* 0)
...
(defun bc-bookmark-new (anno type filename position)
...
POSITION the positio of the breadcrumb bookmark."
  (cons anno  (cons type (cons filename position))))

(defun bc-bookmark-annotation (bookmark)
  (car bookmark)
  )
...
(defun bc-bookmark-filename (bookmark)
  (car  (cdr (cdr bookmark)))
  )

(defun bc-bookmark-position (bookmark)
  (cdr (cdr (cdr bookmark)))
  )
  ...
  (save-excursion
    (save-window-excursion
      (let ((index 0))
        ...
        (mapc
         (lambda (bookmark)
           (insert (format " %s %-60s %-5s            %s\n"
                           (cond ((eq index *bc-current*) "✔") (t "  "))
                           (s-right 60 (abbreviate-file-name (bc-bookmark-filename bookmark)))
                           (bc-bookmark-position bookmark)
                           (bc-bookmark-annotation bookmark)
                           ))
           (setq index (1+ index))
           )
         *bc-bookmarks*)
...
(defun bc-bookmarks-save ()
  ...
  (let ((data-alist
    ...
    (bc-bookmarks-save-file data-alist (bc-get-bookmark-file)))
  )

(defun bc-bookmarks-restore ()
  "Load the bookmarks from file."
  (let ((data-alist (bc-bookmarks-load-file (bc-get-bookmark-file))))
