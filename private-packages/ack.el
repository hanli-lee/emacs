...
\\{ack-minibuffer-local-map}"
  (interactive
   (let ((project-root (or (funcall ack-default-directory-function
     ...
     (list (minibuffer-with-setup-hook (if (>= emacs-major-version 24)
	     ...
	     (ido-completing-read
              (format "Ack in `%s': (default %s) "
                      (file-name-nondirectory
                       (directory-file-name project-root)) symbol)
              ack-history nil nil  nil 'ack-history symbol))
           project-root)
     )
   )
  ...
  (let ((default-directory (expand-file-name
    ...
    (setq last-ack-command command-args)
    (with-current-buffer (compilation-start (if (string= ack-command "ag")
                                                (concat ack-command " -i --hidden '" command-args "' ; exit 1")
                                              (concat ack-command " --nocolor -i '" command-args "' ; exit 1")
                                              )   'ack-mode)
...
(defun ack-filter ()
  (highlight-regexp last-ack-command compilation-warning-face)
  )
