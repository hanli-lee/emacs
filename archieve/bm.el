...
(defun bm-get-repository-file ()
    (concat switch-desktop-session-dir (switch-desktop-get-current-name) "/.bm-repository"))
...
Do nothing if bookmark is present."
  (let((bookmark (bm-bookmark-at (point))))
	...
	(let ((annotation (concat (if (fboundp 'which-function)
                                      (which-function)
                                    "nil"
          ...
          (if current-prefix-arg
              (setq annotation (read-from-minibuffer
                        "annotation: " nil nil nil nil)))
	  (if annotation
              (progn
                (bm-bookmark-annotate bookmark annotation)
                (message (concat "New Bookmark: " annotation))
                )
	      )
	  )
...
users by the likes of `bm-show' and `bm-show-all'."
  ...
  (cl-flet ((format-non-nil (format-string &rest args)
    ...
    (let* ((bookmarks (if lifo-order
           ...
           (format-string (concat (format "%%-%ds" (+ file-line-width 15))
...
(defun bm-repository-load (&optional file)
  ...
  (unless file
    (setq file (bm-get-repository-file)))
  (when file
    (condition-case nil
...
(defun bm-select nil
  (interactive)
  (let ((choices (butlast (s-split "\n" (car (cdr (bm-show-extract-bookmarks bm-in-lifo-order t)))) 1))
	(buffer-name)
	(buffer)
	(bookmark)
	)
    (setq buffer-name (ido-completing-read "Bookmark: " choices))
    (setq buffer-name (nth (-elem-index buffer-name choices) choices))
    (setq buffer (get-text-property 0 'bm-buffer buffer-name))
    (setq bookmark (get-text-property 0 'bm-bookmark buffer-name))
