...
(defun wg-mode-line-string ()
  ...
  (let ((wg (wg-current-workgroup t))
    ...
    (setq total-length (length (wg-workgroup-list-or-error)))
    (setq pos (- total-length (length (member wg (wg-workgroup-list-or-error)))))
    (setq pos (+ 1 pos))
    (cond (wg (wg-fontify " "
                (:brace wg-mode-line-decor-left-brace)
                (:mode (format "%d/%d" pos  total-length ))
                (if wg-flag-modified
                    (concat
...
(defvar wg-prefixed-map
  (wg-fill-keymap
   ...
   ;; workgroup switching
   (kbd "C-j")        'wg-switch-to-workgroup
   (kbd "j")          'wg-switch-to-workgroup
   ...
   (kbd "C-o")        'wg-switch-to-previous-workgroup
   (kbd "o")          'wg-switch-to-previous-workgroup
...
that, use `wg-clone-workgroup'."
  ...
  (wg-switch-to-workgroup (wg-make-and-add-workgroup name t))
  (wg-fontified-message
    (:cmd "Created: ") (:cur name)))
...
(defun wg-kill-workgroup (&optional workgroup)
  ...
  (interactive)
  (let* ((workgroup (wg-get-workgroup workgroup))
	 (wg-count (length (wg-workgroup-list)))
	...
	(progn
      ...
      (message "There's only one workgroup"))))
