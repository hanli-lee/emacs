...
(if auto-highlight-symbol-mode-map
  ...
  (setq auto-highlight-symbol-mode-map
        (let ((map (make-sparse-keymap)))
          ...
          (define-key map (kbd "M-o"    ) 'ahs-occur-hl)
          map)))
...
(defun ahs-copy-hl ()
  (interactive)
  (cond ((region-active-p) (call-interactively 'kill-ring-save))
        (t (copy-region-as-kill ahs-begin ahs-end))))

(defun ahs-kill-hl ()
  (interactive)
  (cond ((region-active-p) (call-interactively 'kill-region))
        (t (kill-region ahs-begin ahs-end))))
